...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	if(R_S_Flag==0)
	{
		...
		memset(text, 0, sizeof(text));
		sprintf(text, "\"SetMode\":{\"value\":%d},",mode);
		strcat(buf, text);
		
		memset(text, 0, sizeof(text));
		sprintf(text, "\"SetTemp\":{\"value\":%d},",temp_Set);
		strcat(buf, text);
		
		memset(text, 0, sizeof(text));
		sprintf(text, "\"SetWindy\":{\"value\":%d},",speed);
		strcat(buf, text);
		
		memset(text, 0, sizeof(text));
		sprintf(text, "\"humi\":{\"value\":%d},",(int32)humi);
		strcat(buf, text);
			
		memset(text, 0, sizeof(text));
		sprintf(text, "\"temp\":{\"value\":%d}",(int32)temp);
		strcat(buf, text);
	...
	else if (R_S_Flag==1)
	{
		...
		int ID=atoi(pure_num);

		strcpy(buf, "{\"id\":");
		
		memset(text, 0, sizeof(text));
		sprintf(text,"\"%s\"",pure_num);
		strcat(buf, text);

		UsartPrintf(USART_DEBUG, "pure_num:%s\r\n",pure_num);

		memset(text, 0, sizeof(text));
		sprintf(text, ",\"code\":200,");
		strcat(buf, text);

		memset(text, 0, sizeof(text));
		sprintf(text, "\"msg\":\"success\"");
		strcat(buf, text);

		UsartPrintf(USART_DEBUG, "Fill2\r\n");
...
void OneNet_SendData(void)
{
	...
	if(body_len)
	{
		if(MQTT_PacketSaveData(PROID, DEVICE_NAME, buf,body_len, NULL, &mqttPacket) == 0)				//封包
		{
//			for(; i < body_len; i++)
...
//				char *data_ptr = NULL;
				...
				cJSON* raw_json=cJSON_Parse(req_payload);										//提取平台下发cjson文本中的数据
				cJSON* id_json=cJSON_GetObjectItem(raw_json,"id");
				cJSON* params_json=cJSON_GetObjectItem(raw_json,"params");
				cJSON* mode_json=cJSON_GetObjectItem(params_json,"SetMode");
				cJSON* temp_json=cJSON_GetObjectItem(params_json,"SetTemp");
				cJSON* windy_cjson=cJSON_GetObjectItem(params_json,"SetWindy");
					
				if(!params_json)
				{
					UsartPrintf(USART_DEBUG,"ERROR:%s",cJSON_GetErrorPtr());
				}
				else
				{
					UsartPrintf(USART_DEBUG,"OKKKKKKKK!!!!!!\r\n");
					if(id_json->type==cJSON_String)
					{
						const char* id_temp=id_json->valuestring;
						strncpy(id,id_temp,sizeof(id)-1);
						UsartPrintf(USART_DEBUG,"id=%s\r\n",id);
					}
					if(mode_json->type==cJSON_Number)
					{
						
						mode=mode_json->valueint;
					}
					if(temp_json->type==cJSON_Number)
					{
						
						temp_Set=temp_json->valueint;
					}
					if(windy_cjson->type==cJSON_Number)
					{
						speed=windy_cjson->valueint;
					}
					Delay_ms(500);
					...
					R_S_Flag=1;	
					ESP8266_Clear();
					OneNet_SendData();	//回复平台
					ESP8266_Clear();
					UsartPrintf(USART_DEBUG, "speed:%d,mode:%d,temp:%d\r\n",speed,mode,temp_Set);
					Midea_Number(speed,mode,temp_Set);
				}
			}
			else
			{
				UsartPrintf(USART_DEBUG, "WARN:	MQTT_UnPacketPublish Err\r\n");
			}
			break;
