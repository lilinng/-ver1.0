...
//	说明：		
//==========================================================
uint1 MQTT_PacketSaveData(const int8 *pro_id, const char *dev_name,const char *payload,
								int16 send_len, int8 *type_bin_head, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(R_S_Flag==0)
	{
		...
		UsartPrintf(USART_DEBUG, "post\r\n");
	}
	else if(R_S_Flag==1)
	{
		UsartPrintf(USART_DEBUG, "set_reply\r\n");
		snprintf(topic_buf, sizeof(topic_buf), "$sys/%s/%s/thing/property/set_reply", pro_id, dev_name);
		R_S_Flag=0;
	}
	else
		return 1;
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, topic_buf,payload, send_len,MQTT_QOS_LEVEL1, 0, 1, mqttPacket) == 0)
	{
